...
def upgrade() -> None:
    ...
    with op.batch_alter_table('sentlog', schema=None) as batch_op:
        ...
        batch_op.create_index(batch_op.f('ix_sentlog_conversation_id'), ['conversation_id'], unique=False)
        batch_op.create_foreign_key(batch_op.f('fk_sentlog_conversation_id_conversation'), 'conversation', ['conversation_id'], ['id'])


def downgrade() -> None:
    """Downgrade schema."""
    with op.batch_alter_table('sentlog', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f('fk_sentlog_conversation_id_conversation'), type_='foreignkey')
