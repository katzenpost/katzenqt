...
def peer_named_in_conversation(
    sess: Session, conversation_id: int, name: str,
) -> ConversationPeer | None:
    """Resolve the single peer named ``name`` that belongs to
    ``conversation_id``, or None when there is no such peer or the name is
    ambiguous.

    Scoping to the conversation is what keeps an announced/inducted peer from
    being tagged with a same-named peer's read cap in another conversation.
    Sync engine: every caller runs on the Qt loop (see
