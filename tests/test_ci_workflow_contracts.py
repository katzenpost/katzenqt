...
def test_listener_configuration_errors_are_not_advisory() -> None:
    ...
    assert "always()" in _step(live, "Upload logs")
...
def test_local_act_uses_persistent_cache_and_skips_remote_artifacts() -> None:
    text = WORKFLOW.read_text(encoding="ascii")
    jobs = _jobs(text)
    for name in ("docker-integration", "epoch-integration"):
        job = jobs[name]
        checkout = _step(job, "Checkout katzenpost")
        assert "clean: ${{ env.KATZENQT_CHECKOUT_CLEAN }}" in checkout
        local_cache = _step(job, "Use the local mixnet cache")
        assert "env.ACT == 'true'" in local_cache
        assert '.ci-local/mixnet-cache' in local_cache
        seed = _step(job, "Set the mixnet base port seed")
        assert '.base_port_seed' in seed
        assert "62331" in seed
        cache = _step(job, "Cache the mixnet build")
        assert "env.ACT != 'true'" in cache
        builder = _step(job, "Configure the local buildx builder")
        assert "docker buildx create" in builder
        assert "--driver-opt default-load=true" in builder
        assert "BUILDX_BUILDER" in builder
        cleanup = _step(job, "Remove the local buildx builder")
        assert "always() && env.ACT == 'true'" in cleanup
        assert "docker buildx rm" in cleanup
    for name in (
        "namenlos-integration", "docker-integration", "epoch-integration",
    ):
        upload_name = (
            "Upload logs" if name == "namenlos-integration"
            else "Upload integration and mixnet logs"
        )
        upload = _step(jobs[name], upload_name)
        assert "always() && env.ACT != 'true'" in upload
