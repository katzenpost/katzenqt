...
def test_voucher_handshake_then_bidirectional(
    ...
    read_bob = _run_role(
        bob_state, "read", "demo", "180", "hello from alice", timeout=240.0
    )
    ...
    _assert_ok(
        _run_role(bob_state, "send", "demo", "hello from bob", timeout=300.0),
        "bob send",
    )
    read_alice = _run_role(
        alice_state, "read", "demo", "180", "hello from bob", timeout=240.0
    )
